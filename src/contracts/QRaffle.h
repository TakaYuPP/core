...
protected:
	...
	PUBLIC_PROCEDURE_WITH_LOCALS(submitProposal)
	{
		...
		{
			if (state.proposals.get(locals.i).proposer == qpi.invocator())
			{
				locals.cnt++;
			}
			if (locals.cnt >= 3)
	...
	PUBLIC_PROCEDURE_WITH_LOCALS(voteInProposal)
	{
		...
		if (state.voteStatus.get(locals.voteKey, locals.existingVote))
		{
			...
			state.voteStatus.replace(locals.voteKey, input.yes);
			output.returnCode = QRAFFLE_SUCCESS;
		...
		state.proposals.set(input.indexOfProposal, locals.proposal);
		state.voteStatus.set(locals.voteKey, input.yes);
	...
	END_EPOCH_WITH_LOCALS()
	{
		...
		if (state.numberOfQuRaffleMembers > 0)
		{
			...
			locals.shareholderRevenue = div<uint64>(state.qREAmount * state.numberOfQuRaffleMembers * QRAFFLE_SHAREHOLDER_FEE, 100);
		...
		for (locals.i = 0 ; locals.i < state.numberOfActiveTokenRaffle; locals.i++)
		{
			if (state.numberOfTokenRaffleMembers.get(locals.i) > 0)
			{
				...
				locals.shareholderRevenue = div<uint64>(locals.acTokenRaffle.entryAmount * state.numberOfTokenRaffleMembers.get(locals.i) * QRAFFLE_SHAREHOLDER_FEE, 100);
